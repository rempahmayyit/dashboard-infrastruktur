...
function PusatDataComponent() {
  ...
  const handleMasterUpload = async (e) => {
    ...
    try {
      ...
      setLogLogs(["Memulai sinkronisasi database pengendalian proyek..."]);
      ...
      reader.onload = async (evt) => {
        try {
          ...
          setExcelData((prev) => {
            const prevRealisasi = prev["db_realisasi"] || [];

            const newRealisasi = allSheets["db_realisasi"] || [];

            // ambil periode upload

            const uploadPeriode = newRealisasi[0]?.periode;

            // hapus periode lama yang sama

            const filteredPrev = prevRealisasi.filter(
              (item) => item.periode !== uploadPeriode,
            );

            // gabungkan

            const mergedRealisasi = [...filteredPrev, ...newRealisasi];

            return {
              ...prev,
              ...allSheets,

              db_realisasi: mergedRealisasi,
            };
          });
          ...
          setLogLogs([`❌ Error membaca file : ${err.message}`]);
        } finally {
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm">
          <div className="flex items-start justify-between mb-8">
            ...
            <div className="w-20 h-20 rounded-3xl bg-blue-50 flex items-center justify-center">
              <Database size={34} className="text-[#000075]" />
            </div>
          </div>
          ...
          <label className="w-full flex items-center justify-center gap-3 py-5 bg-[#000075] text-white text-xl font-bold rounded-3xl hover:bg-blue-900 transition-all cursor-pointer">
            ...
            {isUploading ? "Uploading Database..." : "Upload Database"}
