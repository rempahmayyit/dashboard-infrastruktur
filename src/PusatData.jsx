...
XLSX.writeFile(
  workbook,
  "Master_Template_Seluruh_Modul_Waskita.xlsx"
);
  };
  ...
  { name: "Marketing Pipeline", table: "marketing_pipeline", key: "id" },
...
if (sheet.name === "Time Overrun") {
  data = data.map((row) => ({
    ...
    endDate: row.endDate
            ...
            const { error } = await supabase.from(sheet.table).upsert(data, { onConflict: sheet.key });
            if (!error) summary.push(`✅ Sheet [${sheet.name}] berhasil disinkronkan ke Supabase.`);
            else summary.push(`❌ Sheet [${sheet.name}] gagal: ${error.message}`);
          }
        }
        setLogLogs(summary);
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-fadeIn font-sans">
      <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
        <div className="flex items-center gap-3 border-b border-slate-100 pb-4 mb-6">
          <div className="p-3 bg-blue-50 rounded-xl text-[#000075]"><Database size={24} /></div>
          <div>
            <h2 className="text-xl font-black text-slate-900">Pusat Integrasi & Sinkronisasi Data Awan</h2>
            <p className="text-slate-400 text-xs mt-0.5">Satu pintu pembaruan data untuk seluruh modul portofolio eksekutif Waskita</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="p-5 bg-slate-50 border border-slate-200/60 rounded-2xl flex flex-col justify-between">
            <div>
              <h3 className="font-bold text-slate-800 text-sm">1. Unduh Berkas Induk Berjalan</h3>
              <p className="text-slate-500 text-[11px] mt-1">Dapatkan struktur file Excel terpadu yang memuat konfigurasi kolom baku untuk divisi portofolio.</p>
            </div>
            <button onClick={downloadMasterTemplate} className="mt-5 w-full flex items-center justify-center gap-2 py-2.5 bg-blue-50 text-blue-700 font-bold border border-blue-200 rounded-xl text-xs hover:bg-blue-100 transition-all shadow-sm">
              <Download size={14} /> Unduh Master Template Excel
            </button>
          </div>

          <div className="p-5 bg-slate-50 border border-slate-200/60 rounded-2xl flex flex-col justify-between">
            <div>
              <h3 className="font-bold text-slate-800 text-sm">2. Unggah Master Berkas Konsolidasi</h3>
              <p className="text-slate-500 text-[11px] mt-1">Unggah berkas terpadu yang sudah diperbarui. Mesin akan melakukan parsing multi-sheet ke cloud secara pararel.</p>
            </div>
            <label className="mt-5 w-full flex items-center justify-center gap-2 py-2.5 bg-emerald-600 text-white font-bold rounded-xl text-xs hover:bg-emerald-700 cursor-pointer transition-all shadow-sm text-center">
              {isUploading ? <RefreshCw size={14} className="animate-spin" /> : <Upload size={14} />}
              {isUploading ? "Memproses Pemetaan Cloud..." : "Upload Berkas Induk Keuangan"}
              <input type="file" accept=".xlsx, .xls" onChange={handleMasterUpload} className="hidden" disabled={isUploading} />
            </label>
          </div>
        </div>
        ...
        {logLogs.length > 0 && (
          <div className="mt-6 p-4 bg-slate-900 rounded-xl font-mono text-[11px] text-slate-300 space-y-1.5 shadow-inner">
            <p className="text-slate-500 font-bold border-b border-slate-800 pb-1 flex items-center gap-1"><CheckCircle2 size={12} /> SYSTEM INTEGRITY SYNC LOG:</p>
            {logLogs.map((log, i) => <p key={i}>{log}</p>)}
          </div>
        )}
      </div>
    </div>
