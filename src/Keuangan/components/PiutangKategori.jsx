...
export default function PiutangKategori() {
  ...
  const categorySummary = [
    {
      id: "owner",
      name: "Klaster Owner (Pemerintah/BUMN)",
      total: 1030.5,
      proyekActive: 3,
      color: "#000075",
    },
  ...
  const detailLembarKerja = {
    owner: [
      {
        no: 1,
        proyek: "Probolinggo-Banyuwangi Pkt 3",
        piutang: 366.2,
        retensi: 62.5,
        bruto: 236.2,
        total: 664.9,
        umur: "0-30 hari",
        kendala: "Pekerjaan tanah dalam review MC100.",
      },
      {
        no: 2,
        ...
        total: 185.3,
        umur: "> 90 hari",
        kendala: "Proses amandemen nilai kontrak tambahan.",
      },
      {
        no: 3,
        proyek: "Irigasi Belitang Lempuing Pkt 2",
        piutang: 52.1,
        retensi: 42.7,
        bruto: 85.5,
        total: 180.3,
        umur: "> 180 hari",
        kendala: "Verifikasi dokumen pembayaran oleh PPK.",
      },
    ],
    pengembangan: [
      {
        no: 1,
        proyek: "Tol Ciawi Sukabumi Seksi 3A",
        piutang: 66.5,
        retensi: 33.5,
        bruto: 136.2,
        total: 236.2,
        umur: "30-90 hari",
        kendala: "Proses kelengkapan backup fisik konsultan.",
      },
      {
        no: 2,
        proyek: "Tol Ciawi Sukabumi Seksi 3B",
        piutang: 115.0,
        retensi: 45.8,
        bruto: 150.0,
        total: 310.8,
        umur: "30-90 hari",
        kendala: "Administrasi penagihan termin awal.",
      },
    ],
    non_pengembangan: [
      {
        ...
        total: 105.5,
        umur: "> 180 hari",
        kendala: "Dispute sharing dana dengan member KSO.",
      },
      {
        no: 2,
        proyek: "Struktur Jembatan Musi",
        piutang: 68.0,
        retensi: 42.3,
        bruto: 70.0,
        total: 180.3,
        umur: "> 360 hari",
        kendala: "Menunggu pencairan anggaran daerah.",
      },
    ],
    pdpk: [
      {
        no: 1,
        proyek: "Proyek Infrastruktur Lama X (Dispute)",
        piutang: 50.0,
        retensi: 30.0,
        bruto: 70.0,
        total: 150.0,
        umur: "> 360 hari",
        kendala: "Sedang dalam tahap mediasi hukum/arbitrase.",
      },
      {
        no: 2,
        proyek: "Proyek Y (Klaim Kontraktor)",
        piutang: 26.2,
        retensi: 20.0,
        bruto: 40.0,
        total: 86.2,
        umur: "> 360 hari",
        kendala: "Proses verifikasi ulang dokumen opname fisik.",
      },
    ],
  };
  ...
  const totalPiutangSub = activeData.reduce(
    (sum, item) => sum + item.piutang,
    0,
  );
  const totalRetensiSub = activeData.reduce(
    (sum, item) => sum + item.retensi,
    0,
  );
        ...
        <div className="w-full lg:w-[40%] bg-white rounded-2xl p-4 border border-slate-200 flex flex-col justify-between min-h-[220px]">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 justify-center text-[9px] font-bold text-slate-500">
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#000075]" /> Owner
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#00A4EF]" /> Peng.
              Bisnis
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#64748b]" /> Non-Peng.
            </div>
            <div className="flex items-center gap-1">
              <span className="w-2 h-2 rounded-full bg-[#BD002F]" /> PDPK
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between border-b border-slate-200 pb-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black text-slate-900 uppercase tracking-wider">
              Lembar Kerja:{" "}
              {categorySummary.find((c) => c.id === activeSubTab)?.name}
            </span>
          </div>
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead className="bg-slate-50 text-slate-500 font-semibold border-b border-slate-200">
              <tr>
                ...
                <th className="p-3 w-64 border-r border-slate-200">
                  Nama Proyek Kontraktor
                </th>
                <th className="p-3 text-right border-r border-slate-200 bg-red-50/30 text-red-900">
                  Piutang (Termin)
                </th>
                <th className="p-3 text-right border-r border-slate-200 bg-blue-50/30 text-blue-900">
                  Piutang Retensi
                </th>
                <th className="p-3 text-right border-r border-slate-200 bg-emerald-50/30 text-emerald-900">
                  Tagihan Bruto (WIP)
                </th>
                <th className="p-3 text-right border-r border-slate-200 bg-slate-100/50 font-bold text-slate-900">
                  Jumlah Total
                </th>
                <th className="p-3 text-center border-r border-slate-200 w-24">
                  Klaster Umur
                </th>
                <th className="p-3 pl-4">
                  Kendala Utama / Rencana Tindak Lanjut
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-medium text-slate-700">
              {activeData.map((row) => (
                ...
                >
                  <td className="p-3 text-center font-mono text-slate-400 border-r border-slate-200">
                    {row.no}
                  </td>
                  <td className="p-3 font-bold text-slate-800 border-r border-slate-200">
                    {row.proyek}
                  </td>
                  <td className="p-3 text-right font-mono text-red-600 border-r border-slate-200">
                    {row.piutang.toFixed(1)}
                  </td>
                  <td className="p-3 text-right font-mono text-blue-600 border-r border-slate-200">
                    {row.retensi.toFixed(1)}
                  </td>
                  <td className="p-3 text-right font-mono text-emerald-600 border-r border-slate-200">
                    {row.bruto.toFixed(1)}
                  </td>
                  <td className="p-3 text-right font-mono font-black text-slate-900 border-r border-slate-200 bg-slate-50/40">
                    {row.total.toFixed(1)}
                  </td>
                  <td className="p-3 text-center border-r border-slate-200">
                    <span
                      className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${
                        row.umur.includes("0-30")
                  ...
                  <td
                    className="p-3 pl-4 text-slate-500 italic max-w-xs truncate"
                    title={row.kendala}
                  >
                    {row.kendala}
                  </td>
            ...
            <tfoot className="bg-slate-100 font-black text-slate-900 border-t-2 border-slate-300">
              <tr>
                ...
                <td className="p-3 text-right font-mono text-red-700 border-r border-slate-200">
                  {totalPiutangSub.toFixed(1)}
                </td>
                <td className="p-3 text-right font-mono text-blue-700 border-r border-slate-200">
                  {totalRetensiSub.toFixed(1)}
                </td>
                <td className="p-3 text-right font-mono text-emerald-700 border-r border-slate-200">
                  {totalBrutoSub.toFixed(1)}
                </td>
                <td className="p-3 text-right font-mono text-slate-900 border-r border-slate-200 bg-slate-200/50">
                  {totalJumlahSub.toFixed(1)}
                </td>
