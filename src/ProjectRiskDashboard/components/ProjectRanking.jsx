  ...
  const sortedProjects = useMemo(() => {
    ...
    const calculated = filtered.map((proj) => {
      ...
      const projRealisasi = rawRealisasi.filter(r => {
        ...
        return rId === id && y === selectedYear && m > 0 && m <= selectedMonth;
      }).sort((a, b) => {
        const dateA = a.periode ? new Date(a.periode) : new Date(a.tahun, a.bulan_index - 1);
        const dateB = b.periode ? new Date(b.periode) : new Date(b.tahun, b.bulan_index - 1);
        return dateB - dateA; // Urutkan Descending (Data terbaru di atas)
      });

      const latestReal = projRealisasi[0] || {}; // Ambil data bulan terakhir yang tersedia

      // --- PARAMETER 1: Behind Schedule (Plan vs Actual) ---
      const progRencana = safeParseNumber(latestReal.prog_rencana || proj.progress_rencana);
      const progRealisasi = safeParseNumber(latestReal.prog_real || proj.progress_realisasi);
      const behindSchedule = progRealisasi - progRencana; // Minus = terlambat

      // --- PARAMETER 2: Cost Overrun (MAPP vs Real BKPU) ---
      const nilaiKontrak = safeParseNumber(proj.nk_current || proj.nilai_kontrak);
      const bkMappKumulatif = safeParseNumber(proj.bk_mapp_kumulatif_current);
      const mappPercent = nilaiKontrak > 0 ? (bkMappKumulatif / nilaiKontrak) * 100 : 0;
      const realBkpuPercent = safeParseNumber(latestReal.bkpu_real_kumulatif);
      const costOverrun = mappPercent - realBkpuPercent; // Minus = realisasi melebihi batas MAPP

      // --- PARAMETER 3: Time Overrun / Urgency Ratio ---
      const sisaProgres = Math.max(0, 100 - progRealisasi);
      const endDate = safeDateConvert(proj.end_date_current || proj.end_date);
      let sisaHari = 30; // Fallback jika tidak ada tanggal
      ...
      // Rumus Urgensi: Sisa Progres / Sisa Hari (Makin tinggi makin gawat)
      const urgencyRatio = sisaHari > 0 ? (sisaProgres / sisaHari) * 100 : (sisaProgres * 2);
    ...
    return calculated.sort((a, b) => {
      if (sortBy === "behind_schedule") return a.behindSchedule - b.behindSchedule; // Minus terbesar di atas
      if (sortBy === "cost_overrun") return a.costOverrun - b.costOverrun; // Minus terbesar di atas
      if (sortBy === "time_overrun") return b.urgencyRatio - a.urgencyRatio; // Ratio tertinggi di atas
      return 0;
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-400 text-xs mt-0.5">
            Daftar peringkat kritis proyek On Going berdasarkan deviasi minus terbesar (Data YTD)
          </p>
        </div>
        ...
        <table className="w-full min-w-[1100px] text-left text-xs">
          <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 font-bold">
            <tr>
              ...
              {/* KOLOM PARAMETER 1 */}
              <th className={`p-3 px-4 text-right cursor-pointer hover:bg-slate-100 transition-all ${sortBy === "behind_schedule" ? "bg-blue-50/50 text-[#000075]" : ""}`} onClick={() => setSortBy("behind_schedule")}>
              ...
              {/* KOLOM PARAMETER 2 */}
              <th className={`p-3 px-4 text-right cursor-pointer hover:bg-slate-100 transition-all ${sortBy === "cost_overrun" ? "bg-red-50/50 text-[#BD002F]" : ""}`} onClick={() => setSortBy("cost_overrun")}>
                <div className="flex items-center justify-end gap-1">
              ...
              {/* KOLOM PARAMETER 3 */}
              <th className={`p-3 px-4 text-center cursor-pointer hover:bg-slate-100 transition-all ${sortBy === "time_overrun" ? "bg-orange-50/50 text-orange-600" : ""}`} onClick={() => setSortBy("time_overrun")}>
              ...
              {/* KOLOM PLACEHOLDER BARU */}
              <th className="p-3 px-4 text-right text-slate-400 font-medium bg-slate-100/30">Tagihan Bruto</th>
              <th className="p-3 px-4 text-right text-slate-400 font-medium bg-slate-100/30 border-r border-transparent">Komitmen Cash In</th>
            </tr>
          </thead>
            ...
            {sortedProjects.length === 0 ? (
              <tr>
                <td colSpan={8} className="p-8 text-center text-slate-400 font-medium">
                  Tidak ada proyek dengan status On Going atau SAP Not Updated yang ditemukan pada bulan ini.
                </td>
              </tr>
            ) : (
              sortedProjects.map((proj, idx) => {
                ...
                if (rankNum === 1) rankBadgeColor = "bg-red-600 text-white font-black";
                else if (rankNum === 2) rankBadgeColor = "bg-orange-500 text-white font-black";
                else if (rankNum === 3) rankBadgeColor = "bg-amber-500 text-white font-black";

                return (
                  <tr key={proj.id || idx} className="hover:bg-slate-50/80 transition-all">
                    
                    {/* NOMOR URUT RANKING */}
                    <td className="p-3 px-4 text-center font-bold">
                      <span className={`inline-flex items-center justify-center w-5 h-5 rounded-md text-[10px] ${rankBadgeColor}`}>
                        {rankNum}
                      </span>
                    </td>
                    ...
                    <td className="p-3 px-4 max-w-[320px]">
                      <div className="font-bold text-slate-800 break-words whitespace-normal">{proj.nama_projek}</div>
                      <div className="text-[10px] text-slate-400 mt-0.5">{proj.divisi}</div>
                    </td>
                    ...
                    <td className={`p-3 px-4 text-right font-semibold ${sortBy === "behind_schedule" ? "bg-blue-50/20" : ""}`}>
                      <div className={proj.behindSchedule < 0 ? "text-red-600 font-bold" : "text-emerald-600 font-bold"}>
                    ...
                    <td className={`p-3 px-4 text-right font-semibold ${sortBy === "cost_overrun" ? "bg-red-50/20" : ""}`}>
                      <div className={proj.costOverrun < 0 ? "text-red-600 font-bold" : "text-emerald-600 font-bold"}>
                    ...
                    <td className={`p-3 px-4 text-center font-semibold ${sortBy === "time_overrun" ? "bg-orange-50/20" : ""}`}>
                      <div className="flex items-center justify-center gap-1.5">
                        <span className={`px-2 py-0.5 rounded-lg font-black text-[10px] ${
                          proj.urgencyRatio > 150 ? "bg-red-100 text-red-700 border border-red-200" : proj.urgencyRatio > 80 ? "bg-orange-100 text-orange-700 border border-orange-200" : "bg-emerald-50 text-emerald-700 border border-emerald-200"
                        }`}>
                    ...
                    {/* PLACEHOLDER PARAMETER BARU */}
                    <td className="p-3 px-4 text-right text-slate-400 font-medium bg-slate-50/30">-</td>
                    <td className="p-3 px-4 text-right text-slate-400 font-medium bg-slate-50/30">-</td>
