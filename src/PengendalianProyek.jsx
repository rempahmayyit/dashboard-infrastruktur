    ...
    <div className="space-y-4 animate-fadeIn font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        ...
        <div
          ...
          className={`bg-white rounded-2xl border p-5 shadow-sm cursor-pointer transition-all hover:shadow-md ${
            activeChartTab === "PU"
              ? "ring-2 ring-[#000075] border-[#000075]"
              : "border-slate-200"
          }`}
        >
          ...
          <h2 className="text-3xl font-bold text-slate-800 mb-2">Rp 1.25 T</h2>

          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">RKAP: Rp 1.30 T</span>

            <span className="text-red-500 font-semibold">-3.8%</span>
          </div>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1">Capaian terhadap RKAP</p>
          </div>
        </div>
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-slate-800 mb-2">82%</h2>

          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">RKAP: 85%</span>
          ...
          <div className="mt-4">
            ...
            <p className="text-xs text-slate-400 mt-1">Margin operasional</p>
          </div>
        </div>
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-slate-800 mb-2">Rp 240 M</h2>

          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">RKAP: Rp 260 M</span>

            <span className="text-red-500 font-semibold">-7.6%</span>
          </div>
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-slate-800 mb-2">Rp 180 M</h2>

          <div className="flex items-center justify-between text-sm">
            <span className="text-slate-500">RKAP: Rp 200 M</span>
      ...
      <div className="border-b border-slate-200 pb-3 mt-4">
        ...
        <p className="text-slate-500 text-xs mt-0.5">
          Lembar Kerja Evaluasi Kritis Produksi Divisi Infrastruktur s.d Bulan
          Ini
        </p>
      </div>
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                ...
                {timeOverrunData.map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    <td className="p-2 pl-3 font-bold text-slate-800 truncate max-w-[110px]">
                      {p.name}
                    </td>
                    ...
                    <td className="p-2 text-center font-black text-red-600 pr-3">
                      {p.remain}
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left text-[11px] border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                {behindScheduleData.map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    <td className="p-2.5 pl-3 font-bold text-slate-800 truncate max-w-[120px]">
                      {p.name}
                    </td>
                    <td className="p-2.5 text-center text-slate-500">{p.ra}</td>
                    <td className="p-2.5 text-center text-[#000075]">{p.ri}</td>
                    <td className="p-2.5 text-center font-black text-red-600 pr-3">
                      {p.dev}
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left text-[11px] border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                {costOverrunData.map((p, i) => (
                  <tr key={i} className="hover:bg-slate-50 transition-colors">
                    <td
                      className="p-2 pl-3 font-bold text-slate-800 truncate max-w-[100px]"
                      title={p.name}
                    >
                      {p.name}
                      {p.id === "BIM" && (
                        <span className="ml-1 text-[8px] bg-red-100 text-red-700 px-1 rounded font-black uppercase tracking-wider animate-pulse">
                          BIM
                        </span>
                      )}
                    </td>
                    ...
                    <td className="p-2 text-center text-[#000075] font-bold">
                      {p.real}
                    </td>
                    <td className="p-2 text-center font-black text-red-600 pr-3 bg-red-50/20">
                      {p.dev}
                    </td>
                  </tr>
                ))}
      ...
      <div className="p-4 bg-blue-50/60 border border-blue-100 rounded-xl flex items-center gap-3">
        ...
        <p className="text-slate-700 text-xs font-medium">
          ...
          Sebanyak 22 proyek terindikasi pembengkakan anggaran operasional kerja
          lapangan (
          <span className="italic font-bold">BK/PU &gt; Batas MAPP</span>).
          Diperlukan evaluasi skema subkontraktor dan percepatan penyerapan
          termin klaim sesegera mungkin.
        </p>
      </div>
    </div>
  );
