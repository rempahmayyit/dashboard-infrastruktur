    ...
    switch (activeChartTab) {
      ...
      case "BKPU":
        return {
          name: "BK/PU (%)",
          color: "#f97316",
          title: "Top 5 BK/PU Deviasi",
        };
      case "LB":
        return {
          name: "Laba Bersih (Miliar)",
          color: "#059669",
          title: "Top 5 LB Tidak Tercapai",
        };
      default:
  ...
  useEffect(() => {
    try {
      ...
      if (rawRealisasiData.length === 0) {
        console.warn(
          "PERINGATAN: Data 0! Cek apakah tabel 'db_realisasi' di Supabase kosong, atau gembok RLS Policy (SELECT -> anon) belum dibuka!",
        );
      } else {
        console.log(
          "Daftar Kolom Anda di Supabase:",
          Object.keys(rawRealisasiData[0]).join(", "),
        );
      }
      // ======================================================================
        ...
        const filterDataBulanan = (rawDataArray) => {
          return rawDataArray.filter((item) => {
            ...
            if (itemMonth === 0 && item.bulan) {
              const namaBulan = String(item.bulan)
                .toLowerCase()
                .substring(0, 3);
              const arrayBulan = [
                "jan",
            ...
            const itemType = String(item.nonjo_joi || "")
              .trim()
              .toLowerCase();
            ...
            if (filterNonJo && !filterJoi) {
              isTypeMatch = category === "NON JO" || category.includes("NON");
            }

            // JOI aktif
            else if (!filterNonJo && filterJoi) {
              isTypeMatch = category === "JOI" || category.includes("JOI");
            }
      ...
      <div className="space-y-4 font-sans mt-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 mb-8 items-stretch">
          ...
          <div className="h-full">
            <FinancialWarningList current={current} />
          </div>
        </div>
      </div>
