...
export default function Sidebar({
  ...
  return (
    <div
      className={`${isCollapsed ? "w-20 p-3" : "w-72 p-5"} h-screen print:hidden bg-[#00005a] text-slate-200 flex flex-col justify-between font-sans border-r border-slate-800/40 shadow-2xl sticky top-0 z-10 transition-all duration-300 overflow-hidden shrink-0`}
    >
      <div className="flex flex-col flex-1 overflow-hidden">
              ...
              <div className="flex items-center justify-between px-2 pt-1">
                ...
                <div className="flex items-center justify-center w-[48%]">
                  <img
                    src={waskitaLogo}
                    alt="Waskita"
                    className="h-[40px] object-contain scale-[0.92]"
                  />
                </div>
              </div>
              <div className="border-t border-slate-200 mt-2 mb-1"></div>
              <div className="text-center pb-1">
                <p className="text-[12px] font-black tracking-[0.25em] text-[#00005a] uppercase leading-tight">
                  Divisi
                  <br />
                  Infrastruktur
                </p>
        ...
        {/* KOTAK PROFIL USER */}
        <div
          className={`flex items-center ${isCollapsed ? "justify-center px-0 py-2.5" : "gap-3 px-4 py-3"} mb-4 bg-white/5 rounded-2xl border border-white/10 backdrop-blur-sm transition-all shrink-0`}
        >
                ...
                {item.children ? (
                  <div>
                    ...
                    >
                      ...
                      {!isCollapsed && (
                        <ChevronRight
                          size={14}
                          className={`transition-all duration-300 shrink-0 ${openMenu === item.name ? "rotate-90 text-blue-300" : "text-blue-300/50"}`}
                        />
                      )}
                    </button>
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <ChevronRight
                        size={14}
                        className={`transition-all shrink-0 ${isActive ? "text-white" : "text-blue-300/50"}`}
                      />
                    )}
                  </button>
                )}
      ...
      <div className="mt-auto pt-3 pb-2 px-4">
        <div className="text-[10px] font-black tracking-[0.42em] text-slate-400 text-center uppercase leading-none">
          #FORBETTERWASKITA
        </div>

        <div className="mt-1 flex justify-center">
          <div className="px-2.5 py-[2px] rounded-full bg-white/5 border border-white/10 text-[8px] text-slate-400 font-mono tracking-wide leading-none">
