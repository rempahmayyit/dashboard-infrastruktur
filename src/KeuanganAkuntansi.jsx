      ...
      <div className="dashboard-card flex flex-col xl:flex-row gap-6 items-stretch">
        ...
        <div className="w-full xl:w-[40%] flex flex-col gap-6 justify-between">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody>
                  <tr className="bg-white font-bold text-slate-800">
                    ...
                    <td className="p-3 text-center"><span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded font-black">106,61%</span></td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200 bg-gradient-to-br from-slate-50 to-white flex-1 flex flex-col justify-between">
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-3">Resume Posisi Cash Flow (April)</h3>
            <div className="space-y-2.5 font-mono text-xs">
              <div className="flex justify-between border-b border-slate-100 pb-1.5 text-slate-600"><span>Saldo Awal</span><span className="font-bold text-slate-900">1.979.318 M</span></div>
              <div className="flex justify-between border-b border-slate-100 pb-1.5 text-emerald-600"><span>Cash In April</span><span className="font-bold">(+) 185.928 M</span></div>
              <div className="flex justify-between border-b border-slate-100 pb-1.5 text-red-600"><span>Cash Out April</span><span className="font-bold">(-) 264.175 M</span></div>
              <div className="flex justify-between pt-2 text-sm font-black text-[#000075] border-t-2 border-slate-300">
                <span className="font-sans uppercase">Saldo Akhir</span><span>1.904.966 M</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden bg-white h-full flex flex-col">
            <div className="bg-slate-50 p-3 border-b border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold text-slate-800 uppercase tracking-wide">Komitmen Penerimaan Termin Mei '26 (Total: 176.627 M)</span>
              </div>
            </div>
                  ...
                  {meiCommitmentProjects.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/80 transition-colors group">
                      <td className="p-2.5 pl-3">
                        <div className="font-bold text-slate-800 truncate max-w-[210px]" title={p.name}>{p.name}</div>
                        <div className="text-[9px] text-slate-400 font-mono italic truncate max-w-[210px] group-hover:text-blue-600" title={p.kendala}>{p.kendala}</div>
                      </td>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-200">
        <div className="mb-4 flex flex-col sm:flex-row justify-between sm:items-center gap-2">
          <div>
            ...
            <p className="text-slate-400 text-xs mt-0.5">Penarikan manual klaster jatuh tempo per 04 Mei 2026 (Milyar)</p>
          </div>
        </div>
